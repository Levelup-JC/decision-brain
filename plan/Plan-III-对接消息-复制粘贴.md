...
| **A** | （填名字） | DeepSeek 真 key 注入 + 线上 LLM 留档 + 改提交说明 | §3 A组 / §4 TA-III | 等 C-III-3 部署做实后 |
...
- C 部署做实（线上 /api/health 不再 404）之后，A 组注入真 key、B 组线上复验，并行。
...
1. **注入 DeepSeek 真 key**——Vercel env 里 `LLM_API_KEY` 现在是占位值，用 `vercel env` 替换成真 DeepSeek key。只进 env，绝不进 git。
