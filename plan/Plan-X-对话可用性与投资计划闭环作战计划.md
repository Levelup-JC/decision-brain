# Plan X — 对话可用性与投资计划闭环作战计划

> **权威版本:** 本文件是 Plan X 后续执行与验收的唯一入口。`Plan-X-对话可用性与决策闭环作战计划.md` 视为过程草案；A-E/F 组派发、验收、汇报均以本文件为准。
...
| F 组 | 验收守门与官网证据：统一 Plan X 脚本、本地/公网复跑、截图与总报告 | `tests/plan10-*.mjs`, `plan/Plan-X-F组-任务汇报.md`, `plan/Plan-X-验收总报告.md`, `data/plan10-*.json` |
...
**目标:** 用脚本、截图、JSON 报告证明官网对话真正可用，而不是只证明 API 200 或单个本地 mock 通过。

### Task F1: 建立 Plan X 统一验收脚本

- [ ] 创建或整合 `tests/plan10-dialog-acceptance.mjs`，覆盖以下 9 个 case：

| Case | 输入 | 断言 |
|------|------|------|
| X-01 | `BTC 是什么` | 本地/公网 P95 达标；reply 含价格/市值/FDV；trace 可追溯 |
| X-02 | `ENA FDV 多少` | 只输出可追溯 FDV/市值类事实，不夹带无来源目标价 |
| X-03 | `我想买 SOL，帮我做计划` | 进入首次建仓引导；只问一个下一步问题 |
| X-04 | `我买了 SOL 100 个，成本 120` | 生成或更新 draft plan |
| X-05 | `确认 SOL 计划` | plan.status 变为 active |
| X-06 | `我的持仓总览` | 返回全部仓位 + 每个计划状态 |
| X-07 | `我之前 SOL 的投资计划是什么` | 返回 SOL plan summary |
| X-08 | `检查一下 SOL 计划` | 返回 monitorStatus 与当前 vs 计划对比 |
| X-09 | 坏 MCP URL 后 `BTC 是什么` | 不编数字，trace 红态，回复解释数据不可用 |

### Task F2: 官网 UI 截图证据

- [ ] 用 Playwright 或人工截图保存以下 5 张图：
...
- [ ] 汇总 A-E 组报告，生成 `plan/Plan-X-验收总报告.md`。
- [ ] 总报告必须包含第 10 节的性能对比表、四条能力清单、证据索引。
- [ ] 如果任一 P0 指标未达标，总报告必须标 FAIL，不能写"基本完成"。

### F 组提交标准

- `Plan-X-F组-任务汇报.md` 必须列出 A-E 每组 PASS/FAIL。
- 必须产出：
  - `data/plan10-acceptance-local-*.json`
  - `data/plan10-acceptance-public-*.json`
  - `plan/Plan-X-验收总报告.md`
- 公网连续 2 次通过率必须 >= 95%，且 X-01 / X-03 / X-06 / X-07 / X-08 / X-09 必须 100% 通过。
- agent 超时只有在主回复可用、timeout agent 红态可见、已返回 agent 结果未丢失时才允许判 PASS。

---

## 9. 最终联调顺序

### 9.1 合并顺序（避免 chat-orchestrator 冲突）
...
- [ ] F 组：`node tests/plan10-dialog-acceptance.mjs --http=http://localhost:4177` 通过率 100%。
...
- [ ] F 组在公网页面截图：单资产快查 trace、首次向导、持仓总览、监控对比、超时非阻塞。
...
- F 组：`plan/Plan-X-F组-任务汇报.md`
- 总报告（汇报给你）：`plan/Plan-X-验收总报告.md`

每份组报告必须包含第 7.2 节统一格式。
