...
//
// Usage:
//   node tests/plan9-c-ui-screenshots.mjs
//   node tests/plan9-c-ui-screenshots.mjs --url=http://localhost:4177
//   SERVER_URL=http://localhost:4177 node tests/plan9-c-ui-screenshots.mjs
...
const SCREENSHOT_DIR = join(PROJECT_ROOT, "..", "plan", "Plan-IX-C组-截图");
...
async function testC2(page, url) {
  ...
  await page.route("**/api/chat", async (route) => {
    ...
    if (msg.includes("BTC 是什么") || msg.includes("BTC")) {
      await route.fulfill({
        status: 200,
...
async function collectApiEvidence(page, url) {
  console.log(`\n── Collecting real API response evidence ──`);
  ...
  if (capturedResponse) {
    ...
    console.log(`  API evidence: ${JSON.stringify(evidence, null, 2)}`);
    return evidence;
  }
...
async function main() {
  ...
  try {
    ...
    {
      const page = await context.newPage();
      evidence.apiEvidence = await collectApiEvidence(page, url);
      await page.close();
    }
  } finally {
    await browser.close();
  ...
  console.log(`API evidence:\n${JSON.stringify(evidence.apiEvidence, null, 2)}`);
