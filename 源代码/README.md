...
5. **投资初心护栏（Thesis Guard）**：用户恐慌卖出时，系统先回看原目标、当前进度、投资 thesis、计划边界和底仓规则，区分情绪化卖出和理性卖出，给出 2-3 个克制选项。
6. **卖出意图四层区分**：想卖 → 因下跌想卖 → 准备卖 → 已经卖，每层不同响应策略。
...
- 测试套件：210 测试用例全部通过（54 core + 7 plan12 + 35 plan14 + 64 plan15 + 50 plan16）
